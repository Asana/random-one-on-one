...
WORK_SPACE_GID = None
MANAGE_ONE_ON_ONE_PROJECT_EXPECTED_TASK_NAME = "Hi there! I am the Random one on one bot :)"
...
def get_custom_field_value(member, name, default=None):
    ...
    return next(
        (
            field['enum_value'] for field in member['custom_fields']
...
def create_one_on_one_tasks(client, this_week_id, members_tasks, unmatched_member_task):
    ...
    while len(members_tasks) > 1:
        ...
        one_on_one_task = client.tasks.add_subtask(
            this_week_id, {
                "name":
                    "Random 1:1 for {} : {}".format(name1, name2),
                "notes":
                    "Start by finding a suitable date for your one on one and put it on the calendar",
                "workspace":
                    WORK_SPACE_GID
            }
        )

        _create_one_on_one_subtask(one_on_one_task.get("gid"), member1.get("gid"), name2)
        _create_one_on_one_subtask(one_on_one_task.get("gid"), member2.get("gid"), name1)
    ...
    if unmatched_member_task:
        ...
        client.tasks.add_subtask(
            this_week_id, {
                "name":
                    "Random One on None {}".format(name),
                "assignee":
                    unmatched_member.get("gid"),
                "due_on":
                    TODAY_STR,
                "notes":
                    "Unfortunately you did not get paired this week. \nDon't worry, you will get priority next week! :tada:",
                "workspace":
                    WORK_SPACE_GID
            }
        )
...
def user_is_away(user):
    ...
    end_date = datetime.fromisoformat(vacation_end) if vacation_end is not None else None
...
def generate_random_one_on_one(client, project_gid, members_section, upcoming_section):
    ...
    opt_fields = [
        "assignee", "name", "completed", "custom_fields", "assignee.vacation_dates", "assignee.name"
    ]
    members_tasks = [
    ...
    members_tasks = [
        member for member in members_tasks if member.get("assignee") is not None
        and not member.get('completed') and not user_is_away(member.get("assignee"))
    ...
    matches = ConstructMatches(members_tasks, last_run_matches, member_id, get_custom_field_value)
    ...
    this_week = client.tasks.create_in_workspace(
        WORK_SPACE_GID,
        {
            ...
            "external": {
                "data": json.dumps(matches.match_data)
            }
        }
    )
    ...
    location = {"insert_before": last_run.get("gid")} if last_run else {"section": upcoming_section}
...
def run_for_a_single_project(client, project_id):
    ...
    if members_section and upcoming_section:
        generate_random_one_on_one(client, project_id, members_section, upcoming_section)
    else:
        ...
        if task["name"] == MANAGE_ONE_ON_ONE_PROJECT_EXPECTED_TASK_NAME:
            ...
            if len(task_detailed['projects']) > 1:
                report_error_to_task(
                    task["gid"], "Task lives in more than 1 project.\n"
                ...
                report_error_to_task(task["gid"], "Task doesn't live inside a project :mag:")
            else:
                ...
                if members_section and upcoming_section:
                    ...
                    report_error_to_task(
                        task["gid"], "Missing required sections 'Members' and 'Upcoming'"
                    )
    if len(errors):
...
def report_errors(client, errors):
    ...
    client.tasks.create_in_workspace(
        WORK_SPACE_GID, {
            "name":
                f"Errors creating random one on ones in week [{WEEK_NUMBER}]",
            "html_notes":
                f"<body><em>Encountered error in these 1:1 projects:</em> <ul>{error_list}</ul></body>",
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=
        "Random one on one script. Will generate random one on ones in all projects created from template"
    )
    parser.add_argument(
        "--pat",
        type=str,
    ...
    parser.add_argument(
        ...
        required=True,
        help=
        "User gid used to find incompleted tasks assigned to this user. Runs the script in all projects for tasks named --task-name"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "The id of a particular random 1:1 project. This will only run the script on that project"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "This will use the names of the Member tasks for id's instead of the id's of the assignee, useful when developing as will allow multiple tasks to have the same assignee"
    )
